        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
          <div className="max-w-3xl space-y-2">
            ...
            <p className="text-xs xs:text-sm sm:text-base text-zinc-350 font-bold max-w-2xl leading-relaxed">
              Choose your battlefield. From the 12-hour TECHNOVA Hackathon to
              high-octane E-Football and BGMI showdowns, each mission features
              independent prize pools and dedicated brackets.
            </p>
          </div>
      ...
      {filteredCompetitions.length === 0 ? (
        <div className="text-center py-10 sm:py-12 bg-[#110925]/75 border border-[#ff007f]/30 rounded-lg p-6 sm:p-8">
          ...
          <p className="text-xs text-zinc-400">
            Try adjusting your search criteria or select 'All' to see every
            available mission.
          </p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5 sm:gap-6">
          ...
          {filteredCompetitions.map((comp, idx) => {
            const isThirdCard =
              idx === 2 && filteredCompetitions.length === 3;
            ...
            return (
              ...
              >
                ...
                <div>
                  <div className="relative border-b border-slate-700/60 bg-black flex items-center justify-center overflow-hidden">

                    <img
                      src={comp.image}
                      alt={comp.title}
                      className="w-full h-auto max-h-[650px] object-contain transition-transform duration-300"
                    />
                    ...
                    <div className="bg-black/55 border border-slate-700/60 p-2 text-[10px] sm:text-[11px] font-bold text-zinc-300 space-y-1 mb-2 rounded">
                      <div className="flex items-center gap-1.5">
                        ...
                        <span className="truncate">
                          {comp.schedule.date} • {comp.schedule.time}
                        </span>
                      </div>
                    </div>
                  </div>
